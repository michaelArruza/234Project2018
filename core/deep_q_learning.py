...
class DQN(QN):
    ...
    def get_best_action(self, state):
        ...
        action_values = self.sess.run(self.q, feed_dict={self.s: [state]})[0]#self.sess.run(self.q, feed_dict={self.s: [np.reshape(state, (344,1))]})[0]
    ...
    def update_step(self, t, replay_buffer, lr):
        ...
        #print(s_batch[0], a_batch[0], r_batch[0], sp_batch[0])
        #print np.reshape(sp_batch, (s_batch.shape[0], 343, 1))[0,0:3,:]
        #print a_batch
        #print done_mask_batch
        #print s_batch.shape
        fd = {
            # inputs
            self.s: s_batch,#np.reshape(s_batch, (s_batch.shape[0], 344, 1)),
            self.a: a_batch,
            self.r: r_batch,
            self.sp: sp_batch,#np.reshape(sp_batch, (sp_batch.shape[0], 344, 1)),
