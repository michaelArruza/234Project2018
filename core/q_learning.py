...
class QN(object):
    ...
    def update_averages(self, rewards, max_q_values, q_values, scores_eval):
        ...
        Args:
            ...
            scores_eval: list
        """
        #print rewards, max_q_values, q_values, scores_eval, "HI"
    ...
    def train(self, exp_schedule, lr_schedule):
        ...
        while t < self.config.nsteps_train:
            ...
            self.game.reset()
            state = self.game.get_player_state()#np.array(self.game.get_player_state())
            while True:
                t += 1
                last_eval += 1
                last_record += 1
            #    if self.config.render_train: self.env.render()
                ...
                new_state, reward, done, info = self.game.advance_game(action)#self.env.step(action)
    ...
    #        env = self.env
        ...
        for i in range(num_episodes):
            ...
            while True:
                ...
                # perform action in env
                new_state, reward, done, info = self.game.advance_game(action, evaluate=True)#env.step(action)
