...
class Player():
    ...
    def update_frame(self):
        ...
        self.past.append((self.playerRect.y-(SCREENHEIGHT/2))/(SCREENHEIGHT/2))
        if len(self.past) > 200:
    ...
    def get_state(self):
        return np.array([(self.playerRect.y-(SCREENHEIGHT/2))/(SCREENHEIGHT/2), self.vel/MAXV])
...
class Obstacles():
    ...
    def add_wall(self, level_arr):
        for i in range(len(level_arr)):
            for j in range(len(level_arr[i])):
                if level_arr[i,j] >= 1:
                    self.level_matrix[i,SCREENHEIGHT/BLOCK_SIZE+j] = 1
    ...
    def refresh(self):
        ...
        if self.unseen_counter == UNSEEN:
            self.add_wall((np.random.rand(SCREENHEIGHT/BLOCK_SIZE, UNSEEN) > 0.98).astype(int))
            self.unseen_counter = 0
...
class State():
    def __init__(self):
        ...
        self.obstacleObj.add_wall((np.random.rand(SCREENHEIGHT/BLOCK_SIZE, UNSEEN) > 0.98).astype(int))
    ...
    def advance_game(self, action):
        ...
        if self.fail:
            ...
            reward = 1 - abs(self.playerObj.vel)
            #print reward
