...
class Player():
    ...
    def get_state(self):
        y_state = [self.playerRect.y/300.0, (260.0 - self.playerRect.y - 25.0) / (260.0-28.0), (self.playerRect.y - 28.0)/(260.0-25.0)]
...
class Obstacles():
    def __init__(self):
        ...
        self.setBlocks = []
        self.level_matrix = np.zeros((SCREENHEIGHT/BLOCK_SIZE,SCREENWIDTH/BLOCK_SIZE + UNSEEN))
    ...
    def add_wall(self, level_arr):
        for i in range(len(level_arr)):
            for j in range(len(level_arr[i])):
                if level_arr[i,j] >= 1:
                    self.level_matrix[i,SCREENWIDTH/BLOCK_SIZE+j] = 1.0
    ...
    def refresh(self):
        ...
        if self.unseen_counter == UNSEEN:
            self.add_wall((np.random.rand(SCREENHEIGHT/BLOCK_SIZE, UNSEEN) > 0.988).astype(np.float32))
            self.unseen_counter = 0
...
class State():
    def __init__(self):
        ...
        self.obstacleObj.add_wall((np.random.rand(SCREENHEIGHT/BLOCK_SIZE, UNSEEN) > 0.988).astype(int))
    ...
    def get_player_state(self):
        ...
        #state= np.sum(np.array(pygame.surfarray.pixels3d(self.screen), dtype=np.float32), axis=2)/(3.0*255.0)
        ...
        state = np.copy(self.obstacleObj.get_visible_state())
        x = int(state.shape[1]*0.1)
        f_index = (self.playerObj.playerRect.y+(25/2))/(SCREENHEIGHT/float(state.shape[0]))
        index = int(math.floor(f_index)) - 1
        percentage = f_index - math.floor(f_index)
        state[index,x] = -1.0*(1.0 - percentage)
        if index!= state.shape[0]-1:
            state[index+1,x] = -1.0*percentage
        state[0,:]=1.0
        state[-1,:] = 1.0
        return np.concatenate([state.flatten(), [self.playerObj.vel/5.0]])
    ...
    def advance_game(self, action, evaluate=False):
        ...
        for i in range(5):
            ...
            if self.fail:
                ...
                self.fail = False
                for j in range(5-i):
                    states.append(self.get_player_state())
                break
            else:
...
def main():
    ...
    """
    while(True):
        a = s.advance_game(1 if np.random.rand(1)>0.5 else 0, evaluate=True)
