...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        state_shape = [self.config.player_state_size]#list(self.env.observation_space.shape)
        ...
        player_state = state_shape[0]
       # w = state_shape[1]
       # nchannels = state_shape[2]
        nchannels = 1
        self.s = tf.placeholder(tf.float32, shape=[None, 12*20+1])#player_state, nchannels*config.state_history])
        self.a = tf.placeholder(tf.int32, shape=[None])
        self.r = tf.placeholder(tf.float32, shape=[None])
        self.sp = tf.placeholder(tf.float32, shape=[None,12*20+1])# player_state, nchannels*config.state_history])
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        out = layers.fully_connected(inputs=flattened_state, num_outputs=num_actions, reuse=reuse, scope=scope, activation_fn = None)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        q_target_vars = tf.trainable_variables(target_q_scope)
        ops = [tf.assign(q_target_vars[i], q_vars[i]) for i in range(len(q_vars))]
    ...
    def add_loss_op(self, q, target_q):
        ...
        x = tf.reduce_max(target_q, axis=1)
        gamma = tf.constant(self.config.gamma, dtype=tf.float32)
        x2 = tf.multiply(tf.cast(tf.logical_not(self.done_mask), tf.float32), tf.multiply(x,gamma))
        q_samp = tf.add(x2, self.r)
        a_onehot = tf.one_hot(self.a, num_actions, axis=1)
        self.loss = tf.reduce_mean(tf.square(tf.subtract(q_samp, tf.reduce_sum(tf.multiply(q, a_onehot), axis=[1]))))
    ...
    def add_optimizer_op(self, scope):
        ...
        grads = opt.compute_gradients(self.loss, tf.trainable_variables(scope))
        if self.config.grad_clip:
            grads = [(tf.clip_by_norm(x[0], self.config.clip_val), x[1]) for x in grads]
        #grads[0] = (tf.Print(grads[0][0], [tf.reduce_max([tf.norm(x[0]) for x in grads])]), grads[0][1])
