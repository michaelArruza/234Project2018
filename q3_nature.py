...
class NatureQN(Linear):
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            #x4 = layers.fully_connected(layers.fully_connected(layers.fully_connected(layers.fully_connected(p_state, 100), 100),50),50)
            #out = layers.fully_connected(x4, num_actions, activation_fn = None)
            vel = tf.expand_dims(state[:,-1],-1)
            level = tf.expand_dims(tf.reshape(state[:,:-1],[-1,12, 20]),-1)
            #level = tf.Print(level,[level[0,:,:,0]],summarize=10000)
            first_conv = tf.contrib.layers.conv2d(level, 10, (3,3), stride=2)
            second_conv = tf.contrib.layers.conv2d(first_conv, 20, (2,2), stride=1)
            third_conv = tf.contrib.layers.conv2d(second_conv, 5, (1,1), stride=1)
            print third_conv
            hidden = tf.contrib.layers.fully_connected(tf.contrib.layers.flatten(third_conv), 100)
            hidden2 = tf.contrib.layers.fully_connected(tf.concat([hidden,vel],axis=1), 40)
            out = tf.contrib.layers.fully_connected(hidden2, num_actions,activation_fn = None)
        ##############################################################
        ######################## END YOUR CODE #######################
        return out
