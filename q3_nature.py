...
class NatureQN(Linear):
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            p_state = layers.flatten(state[:,:4,:])
            #p_state = tf.Print(p_state, [p_state[0]])
            l_state = tf.reshape(layers.flatten(state[:,4:,:]),[tf.shape(state)[0],17, 20])
            x1 = layers.conv2d(inputs=l_state, num_outputs=10, kernel_size=2, stride=1)
            x2 = layers.conv2d(inputs=x1, num_outputs=20, kernel_size =2, stride=1)
            x3 = layers.conv2d(inputs=x2, num_outputs=20, kernel_size=1, stride=1)
            x4 = layers.fully_connected(layers.flatten(x3), 100)
            x4 = layers.fully_connected(layers.fully_connected(tf.concat([p_state, x4],axis=1), 60), 40)
            #x4 = layers.fully_connected(layers.fully_connected(layers.fully_connected(p_state, 200), 200),100)
